...
def info_screen(level_num: int, planet_name: str, planet_desc_list, image_path: str, game_bool: bool = True):
    ...
    resized_planet = planet_img.resize((180, 180), Image.LANCZOS)
...
def lobby():
    ...
    if shipno == 6:
        resized_shipimg = shipimg.resize((180, 230), Image.LANCZOS)
    elif shipno == 7:
        resized_shipimg = shipimg.resize((180, 220), Image.LANCZOS)
    elif shipno == 8:
        resized_shipimg = shipimg.resize((120, 250), Image.LANCZOS)
    else:
        resized_shipimg = shipimg.resize((180, 180), Image.LANCZOS)
    shipimg = ImageTk.PhotoImage(resized_shipimg)
    resized_platform = platform.resize((200, 90), Image.LANCZOS)
    platform = ImageTk.PhotoImage(resized_platform)
    resized_controls = controls.resize((320, 230), Image.LANCZOS)
    controls = ImageTk.PhotoImage(resized_controls)
    resized_parts = parts.resize((300, 200), Image.LANCZOS)
    ...
    Label(lobby_screen, text="COSMOFURY", bg="#000", fg="#fff",
...
def Loginform():
    ...
    resized_shipimg = shipimg.resize((160, 160), Image.LANCZOS)
    shipimg = ImageTk.PhotoImage(resized_shipimg)
    ...
    resized_planet1 = saturn.resize((195, 190), Image.LANCZOS)
    planet1 = ImageTk.PhotoImage(resized_planet1)
    resized_planet2 = jupiter.resize((180, 190), Image.LANCZOS)
    planet2 = ImageTk.PhotoImage(resized_planet2)
    resized_planet3 = earth.resize((180, 190), Image.LANCZOS)
    planet3 = ImageTk.PhotoImage(resized_planet3)
    resized_planet4 = mercury.resize((110, 110), Image.LANCZOS)
    planet4 = ImageTk.PhotoImage(resized_planet4)
    resized_planet5 = mars.resize((110, 110), Image.LANCZOS)
    planet5 = ImageTk.PhotoImage(resized_planet5)
    resized_planet6 = venus.resize((110, 110), Image.LANCZOS)
    planet6 = ImageTk.PhotoImage(resized_planet6)
    resized_planet7 = uranus.resize((180, 180), Image.LANCZOS)
    planet7 = ImageTk.PhotoImage(resized_planet7)
    resized_planet8 = neptune.resize((120, 120), Image.LANCZOS)
    ...
    Label(login_screen, text="COSMOFURY", bg="#000", fg="#fff",
...
if __name__ == "__main__":
    conn = conn = dat.connect(
        host="localhost", user="root", passwd="SidharthPriyu", database="GAME")
    cur = conn.cursor()
